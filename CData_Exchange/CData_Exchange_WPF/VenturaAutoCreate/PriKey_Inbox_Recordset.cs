...
{
	/// <summary>
	/// The updateable table is [Exchange].[Inbox]. Updateable table column information:
	...
	///   ItemClass, Subject, Sensitivity, Body, AttachmentId, AttachmentName, DateTimeReceived, Size, Categories, Importance,
	...
	/// </summary>
	public partial class PriKey_Inbox_Recordset : ResultsetObservable<PriKey_Inbox_Recordset, PriKey_Inbox_Record>, IRecordsetBase
	{
		...
		public PriKey_Inbox_Recordset()
		{
			_resultsets = new IResultsetBase[] { this };

			_sqlscript = @"SELECT [ItemId],[MimeContent],[ItemChangeKey],[ParentFolderId],[ParentFolderChangeKey],[ItemClass],[Subject],[Sensitivity],[Body],[AttachmentId],[AttachmentName]," + CRLF +
			...
			schema_array.Add(new VenturaColumn("AttachmentId", typeof(string), true)
			{
				ColumnSize = 2000,
				NumericScale = 0,
				ProviderType = 16,
				...
				BaseSchemaName = "Exchange",
				BaseTableName = "Inbox",
				BaseColumnName = "AttachmentId"
			});

			schema_array.Add(new VenturaColumn("AttachmentName", typeof(string), true)
			{
				ColumnSize = 2000,
				NumericScale = 0,
				ProviderType = 16,
			...
			((IResultsetBase)this).UpdateableTablename = "[Exchange].[Inbox]";
		}

		/// <summary>
		/// Database Column NotUpdateable. Table [Exchange].[Inbox]. PrimaryKey. Readonly. AllowNull.
		...
		/// <summary>
		/// Database Column NotUpdateable. Table [Exchange].[Inbox]. Readonly. AllowNull.
		...
		/// <summary>
		/// Database Column Updateable. Table [Exchange].[Inbox]. NotReadonly. AllowNull.
		...
		{
			get { if (CurrentRecord == null) throw new InvalidOperationException(VenturaStrings.CURRENT_RECORD_NOT_SET); return CurrentRecord.AttachmentId; }
			set { if (CurrentRecord == null) throw new InvalidOperationException(VenturaStrings.CURRENT_RECORD_NOT_SET); CurrentRecord.AttachmentId = value; }
		}
		...
		{
			get { if (CurrentRecord == null) throw new InvalidOperationException(VenturaStrings.CURRENT_RECORD_NOT_SET); return CurrentRecord.AttachmentName; }
			set { if (CurrentRecord == null) throw new InvalidOperationException(VenturaStrings.CURRENT_RECORD_NOT_SET); CurrentRecord.AttachmentName = value; }
		}
		...
		public void Append()
		{
			...
			this.InsertItem(index, new PriKey_Inbox_Record());
			this.CurrentRecordIndex = index;
		}

		public void Append(PriKey_Inbox_Record record)
		{
			int index = this.RecordCount;
		...
		public PriKey_Inbox_Record NewRecord()
		{
			return new PriKey_Inbox_Record();
		}

		protected override PriKey_Inbox_Record InternalCreateExistingRecordObject(object[] columnvalues) => new PriKey_Inbox_Record(columnvalues);

		byte[] IRecordsetBase.Hash
		{
			get { return new byte[] { 118, 227, 202, 221, 199, 33, 14, 25, 93, 70, 166, 80, 154, 255, 121, 146 }; }
		}

		string IRecordsetBase.HashString
		{
			get { return "76E3CADDC7210E195D46A6509AFF7992"; }
		}
	...
	public sealed partial class PriKey_Inbox_Record : IRecordBase, INotifyPropertyChanged
	{
		...
		private string _cur__AttachmentId; private string _ori__AttachmentId; private bool _mod__AttachmentId;
		private string _cur__AttachmentName; private string _ori__AttachmentName; private bool _mod__AttachmentName;
		...
		public PriKey_Inbox_Record()
		{
			...
			_cur__Body = null;
			_cur__AttachmentId = null;
			_cur__AttachmentName = null;
		...
		public PriKey_Inbox_Record(object[] columnvalues)
		{
			...
			_cur__AttachmentId = (string)columnvalues[9];
			_cur__AttachmentName = (string)columnvalues[10];
		...
		public string AttachmentId
		{
			get { return _cur__AttachmentId; }
			set
			{
				if (_started_with_dbvalues == false) _mod__AttachmentId = true;
				if (_cur__AttachmentId == value) return;
				if (_started_with_dbvalues == true)
				{
					if (_mod__AttachmentId == false) { _ori__AttachmentId = _cur__AttachmentId; _mod__AttachmentId = true; } // existing record and column is not modified
					else { if (value == _ori__AttachmentId) { _ori__AttachmentId = default(string); _mod__AttachmentId = false; } } // existing record and column is modified
				}
				_cur__AttachmentId = value; OnPropertyChanged("AttachmentId"); OnAfterPropertyChanged("AttachmentId");
			}
		}
		...
		public string AttachmentName
		{
			get { return _cur__AttachmentName; }
			set
			{
				if (_started_with_dbvalues == false) _mod__AttachmentName = true;
				if (_cur__AttachmentName == value) return;
				if (_started_with_dbvalues == true)
				{
					if (_mod__AttachmentName == false) { _ori__AttachmentName = _cur__AttachmentName; _mod__AttachmentName = true; } // existing record and column is not modified
					else { if (value == _ori__AttachmentName) { _ori__AttachmentName = default(string); _mod__AttachmentName = false; } } // existing record and column is modified
				}
				_cur__AttachmentName = value; OnPropertyChanged("AttachmentName"); OnAfterPropertyChanged("AttachmentName");
			}
		}
		...
		public bool IsModified(string column_name)
		{
			...
			if (column_name == "AttachmentId") return _mod__AttachmentId;
			if (column_name == "AttachmentName") return _mod__AttachmentName;
		...
		public int ModifiedColumnCount()
		{
			...
			if (_mod__AttachmentId == true) count++;
			if (_mod__AttachmentName == true) count++;
		...
		public void ResetToUnmodified()
		{
			if (_started_with_dbvalues == true)
			{
				...
				if (_mod__AttachmentId) _ori__AttachmentId = default(string);
				if (_mod__AttachmentName) _ori__AttachmentName = default(string);
			...
			_mod__Body = false;
			_mod__AttachmentId = false;
			_mod__AttachmentName = false;
...
namespace WpfApp1.VenturaAutoCreate
{
	public partial class PriKey_Inbox_Record
	{
		partial void OnAfterPropertyChanged(string propertyName)
		{
